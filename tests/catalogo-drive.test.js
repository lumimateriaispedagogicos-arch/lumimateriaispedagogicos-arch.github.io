...
(async function () {
  ...
  const falhaComCacheVencido = ambiente(async () => { throw new Error("rede indisponível"); }, cacheVencido);
  falhaComCacheVencido.LUMI_DRIVE_CONFIG = Object.assign({}, falhaComCacheVencido.LUMI_DRIVE_CONFIG,
    { endpoint: "https://example.test/exec" });
  const preservado = await falhaComCacheVencido.CatalogoDrive.carregar([
    { titulo: "Material local", categoria: "Temáticos", arquivo: "materiais/local.pdf" }
  ]);
  assert.equal(preservado.length, 2);
  assert.equal(preservado[0].titulo, "Material local");
  assert.equal(preservado[1].titulo, "Material do cache");
