...
(async function () {
  ...
  const cacheVencidoComFalha = ambiente(async () => { throw new Error("rede indisponível"); }, cacheVencido);
  cacheVencidoComFalha.LUMI_DRIVE_CONFIG = Object.assign({}, cacheVencidoComFalha.LUMI_DRIVE_CONFIG,
    { endpoint: "https://example.test/exec" });
  const preservadoAposFalha = await cacheVencidoComFalha.CatalogoDrive.carregar([]);
  assert.equal(preservadoAposFalha[0].titulo, "Material do cache");

  const semEndpointComCache = ambiente(() => { throw new Error("fetch não deveria ser chamado"); }, cacheVencido);
  const preservadoSemEndpoint = await semEndpointComCache.CatalogoDrive.carregar([]);
  assert.equal(preservadoSemEndpoint[0].titulo, "Material do cache");

  const cacheVencidoComTimeout = ambiente((url, opcoes) => new Promise((resolve, reject) => {
    opcoes.signal.addEventListener("abort", () => reject(new Error("timeout")));
  }), cacheVencido);
  cacheVencidoComTimeout.LUMI_DRIVE_CONFIG = Object.assign({}, cacheVencidoComTimeout.LUMI_DRIVE_CONFIG,
    { endpoint: "https://example.test/exec", timeoutMs: 5 });
  const preservadoAposTimeout = await cacheVencidoComTimeout.CatalogoDrive.carregar([]);
  assert.equal(preservadoAposTimeout[0].titulo, "Material do cache");
