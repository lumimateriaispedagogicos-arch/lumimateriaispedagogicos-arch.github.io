...
(async function () {
  ...
  function renderizar(materiais) {
    ...
    categorias.forEach(function (c, i) {
      ...
      el.className = "card-categoria" + (c.qtd === 0 ? " embreve" : ""); el.style.setProperty("--cor", cor);
      ...
      el.innerHTML = '<span class="cat-icone">' + (ICONES_CATEGORIA[c.nome] || "📚") + "</span><h3>" + escaparHtml(c.nome) + "</h3>" +
        (c.qtd === 0 ? '<span class="cat-qtd">🌱 Em breve!</span>' : '<span class="cat-qtd">' + c.qtd + (c.qtd > 1 ? " atividades" : " atividade") + "</span>");
