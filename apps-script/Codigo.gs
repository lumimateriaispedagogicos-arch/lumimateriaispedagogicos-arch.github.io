...
function obterCatalogo_() {
  const cache = CacheService.getScriptCache();
  const salvo = cache.get(CHAVE_CACHE_CATALOGO);
  if (salvo) {
    try { return JSON.parse(salvo); } catch (erroCache) { /* Reconstrói um cache inválido. */ }
  }
...
function doGet(e) {
  try {
    ...
    return respostaJson_(parametros.action === 'pdf' ? entregarPdf_(parametros.id) : obterCatalogo_());
  } catch (erro) {
