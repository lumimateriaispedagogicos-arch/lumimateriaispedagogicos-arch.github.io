(function () {
  ...
  function lerCache(config, aceitarVencido) {
    try {
      const cache = JSON.parse(localStorage.getItem(CACHE_KEY));
      if (!cache || !Array.isArray(cache.materiais)) return [];
      if (!aceitarVencido && Date.now() - cache.salvoEm > config.cacheMs) return [];
  ...
  async function buscar(config) {
    if (!texto(config.endpoint)) return null;
  ...
  async function carregar(locais) {
    ...
    // O catálogo vencido continua sendo a melhor contingência visual enquanto
    // a rede responde e também depois de timeout/erro. Uma resposta válida é a
    // única situação em que ele deve ser substituído.
    let remotos = lerCache(config, true);
    try {
      const atualizados = await buscar(config);
      if (atualizados !== null) remotos = atualizados;
    }
  ...
  function carregarCache(locais) {
    return mesclar(Array.isArray(locais) ? locais : [], lerCache(configAtual(), true));
  }
  ...
  window.CatalogoDrive = { carregar: carregar, carregarCache: carregarCache, obterPdf: obterPdf,
